...
const KbFieldFactory = (path, type) => new KbField(path, type);

KbFieldFactory._plugin = function(type, options) {
  KbField.prototype.RenderTypes[type] = options;
};

KbFieldFactory._setter = function(name, setter) {
  if (_.isUndefined(setter)) {
    KbField.prototype[name] = function(value) {
      this.options[name] = value;
      return this;
    };
  } else {
    KbField.prototype[name] = setter;
  }
};

export default KbFieldFactory;
